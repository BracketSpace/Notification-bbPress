...
class DumpHooks {
	...
	public function __invoke( $args ) {
		...
		$objects = [];
		foreach ( MainClass::components() as $component_name => $instance ) {
		...
		foreach ( $runtime->get_calls() as $class_name => $hooks ) {
			...
			if ( 'BracketSpace\\Notification\\bbPress\\Runtime' === $class_name ) {
				$callback_object_name = '$this';
			} else {
				$component_name = array_search( $class_name, $objects, true );
				if ( ! $component_name ) {
					WP_CLI::warning( str_replace( 'BracketSpace\\Notification\\bbPress\\', '', $class_name ) . ' skipped, no instance available' );
					continue;
				}
			...
			WP_CLI::log( str_replace( 'BracketSpace\\Notification\\bbPress\\', '', $class_name ) . ' added ' . $count . ' hooks' );
		}
