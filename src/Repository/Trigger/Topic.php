 ...
 * Topic trigger class
 */
abstract class Topic extends BaseTrigger
{
	/**
	 * Forum instance.
	 *
	...
	public function __construct($params = [])
	{
		...
		$this->setGroup(__('bbPress : Topic', 'notification-bbpress'));
	}

	/**
	 * Sets trigger properties.
	 *
	 * @param  \WP_Post  $topic Topic instance.
	 * @return void
	 */
	protected function setProperties($topic)
	{
		...
		$this->addMergeTag(
			new MergeTag\DateTime\DateTime(
				[
					...
					'timestamp' => static function (Topic $trigger) {
		...
		$this->addMergeTag(
			new MergeTag\IntegerTag(
				[
					...
					'resolver' => static function (Topic $trigger) {
		...
		$this->addMergeTag(
			new MergeTag\IntegerTag(
				[
					...
					'group' => __('Topic', 'notification-bbpress'),
					'resolver' => static function (Topic $trigger) {
						return is_array($trigger->meta['_bbp_voice_count'] ?? null)
							? $trigger->meta['_bbp_voice_count'][0]
							: 0;
					},
				]
			)
